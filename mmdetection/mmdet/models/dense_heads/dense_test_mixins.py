...
class BBoxTestMixin(object):
    ...
    def gen_tensor_output_bboxes(self, feats, img_metas, activation=True, rescale=False):
        """Test det bboxes without test-time augmentation, can be applied in
        DenseHead except for ``RPNHead`` and its variants, e.g., ``GARPNHead``,
        etc.
        ...
        outs = self.forward(feats)
        if not activation:
            return [[t.cpu() for t in x] for x in outs]
        results_list = self.get_bboxes_tensor(
            *outs, img_metas=img_metas, activation=activation, rescale=rescale)
        return results_list
    ...
    def simple_test_rpn(self, x, img_metas, activation=True):
        ...
        rpn_outs = self(x)
        if not activation:
            return [[t.cpu() for t in x] for x in rpn_outs]
