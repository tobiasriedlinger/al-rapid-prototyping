...
class YOLOV3Head(BaseDenseHead, BBoxTestMixin):
    ...
    def forward(self, feats, get_features=False):
        ...
        for i in range(self.num_levels):
            ...
            if get_features:
                pred_maps.append(x)
                continue
            if "dropout" in self.train_cfg:
                x = nn.functional.dropout(x, p=self.train_cfg.dropout, training=True)
    ...
    def get_bboxes(self,
        ...
        if cfg.metrics:
            return flatten_bboxes.cpu(), flatten_cls_scores.cpu(), flatten_objectness.cpu()
        det_results = []
    ...
    def get_bboxes_tensor(self,
        ...
        if activation:
            ...
            flatten_objectness = flatten_preds[..., 4]
            flatten_cls_scores = flatten_preds[..., 5:]
            
            return flatten_bbox_preds.cpu(), flatten_cls_scores.cpu(), flatten_objectness.cpu()
