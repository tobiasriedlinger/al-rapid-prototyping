...
from dataset_settings import MNISTDetectionSettings
from box_functions import boxes_too_close, gen_boxes_from_segments
from segment_functions import extract_segments

parser = argparse.ArgumentParser()
parser.add_argument("--split", dest="split", default="dev")
parser.add_argument("--ds-size", dest="dataset_size", default=100, type=int)
parser.add_argument("--img-size", dest="img_size", default=320, type=int)
parser.add_argument("--target-dir", dest="target_dir", default="/home/USR/mnist-det-dataset-lfs")

torch.random.manual_seed(0)

class GenerateMNISTDetection(object):
	def __init__(self,
	             image_size,
	             dataset_size,
				 split="train"
	             ):
		assert split in ["train", "val", "test", "dev"]
		self.split = split
		gen_from_train_set = (split in ["train", "val"])
		self.settings = MNISTDetectionSettings(image_size=image_size, train=gen_from_train_set)
	...
	def generate_set(self,
		...
		while img_id < self.dataset_size:
			...
			if num_instances == 0:
				...
				mnist_indices = torch.multinomial(self.settings.multinom_weights,
												num_samples=int(num_instances),
												replacement=True
												)
				for idx in mnist_indices:
					data = self.settings.dataset[idx]
					img_tensors.append(data[0])
				...
				if len(boxes):
					...
					print(len(boxes), len(img_tensors))
		...
		with open(os.path.join(target_directory,
		                       self.split,
		                       "annotations",
		                       f"mnistdetection_{self.split}_instances.json"), "w") as f:
...
if __name__ == "__main__":
	...
	GenerateMNISTDetection(image_size=args.img_size, dataset_size=args.dataset_size, split=args.split).generate_set(target_directory=args.target_dir)
